...
export default function Checkout() {
  ...
  const onSubmit = async (data: CheckoutFormData) => {
    ...
    toast({
      ...
      description: "Ваш заказ был успешно размещен. Вы получите подтверждение по электронной почте в ближайшее время.",
    });
                ...
                <div className="space-y-3">
                  {items.map((item) => (
                    <div key={item.id} className="flex items-center space-x-3">
                      <img
                        src={item.imageUrl || "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?ixlib=rb-4.0.3&auto=format&fit=crop&w=60&h=60"}
                        alt={item.name}
