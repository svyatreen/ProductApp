    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">Товары</h1>
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6">
            ...
            <Button type="submit" className="sm:px-6">
              ...
              <span className="hidden sm:inline">Искать товары</span>
            </Button>
          </form>
          ...
          {(currentSearch || currentCategory) && (
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <p className="text-gray-600">
                {currentSearch && currentCategory && 
                  `Показаны результаты для "${currentSearch}" в категории "${categories?.find(c => c.id.toString() === currentCategory)?.name}"`
                }
                {currentSearch && !currentCategory && `Показаны результаты для "${currentSearch}"`}
                {currentCategory && !currentSearch && 
                  `Показаны товары в категории "${categories?.find(c => c.id.toString() === currentCategory)?.name}"`
                }
              </p>
              <Button 
                variant="outline" 
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 text-base sm:text-lg">Товары не найдены.</p>
            <p className="text-gray-400 text-sm mt-2">Попробуйте изменить условия поиска или просмотреть все категории</p>
            <Button
              variant="outline"
              onClick={() => navigate("/products")}
              className="mt-4"
