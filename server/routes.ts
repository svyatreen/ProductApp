...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/users/register", async (req, res) => {
    try {
      ...
      // Automatically create vendor profile for new users
      try {
        const vendorData = {
          userId: user.id,
          storeName: `${userData.username || userData.email.split('@')[0]}'s Store`,
          storeDescription: "Welcome to my store! I'm excited to share great products with you.",
          isApproved: true
        };
        await storage.createVendor(vendorData);
      } catch (vendorError) {
        console.log("Note: Could not create vendor profile automatically:", vendorError);
