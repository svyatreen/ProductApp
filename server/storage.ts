...
export class DatabaseStorage implements IStorage {
  ...
  async getFeaturedVendors(): Promise<Vendor[]> {
    return await db.select().from(vendors).where(eq(vendors.isApproved, true)).orderBy(desc(vendors.rating)).limit(6);
  }
  ...
  async removeFromFavorites(userId: number, productId: number): Promise<boolean> {
    const result = await db.delete(favorites).where(and(eq(favorites.userId, userId), eq(favorites.productId, productId)));
    return result.rowCount ? result.rowCount > 0 : false;
  }
  ...
  // Messages
  async getMessages(vendorId: number): Promise<Message[]> {
    return await db.select().from(messages).where(eq(messages.vendorId, vendorId)).orderBy(desc(messages.createdAt));
  }
  ...
  // Suppliers
  async getSuppliers(vendorId: number): Promise<Supplier[]> {
    return await db.select().from(suppliers).where(eq(suppliers.vendorId, vendorId)).orderBy(desc(suppliers.createdAt));
  }
